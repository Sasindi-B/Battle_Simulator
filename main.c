...
{   int battleshipHealth = 100;
    ...
    float ang_min = random (0,29);
    float radian = ang_min* (PI / 180);
    float ang_B = random (ang_min, (PI/2));  // angle which the shells of battleship is projected
    ...
    //Maximum Range of the battleship
    ...
    int Escortships;
    printf("Enter number of escortships: ", Escortships);
    scanf("%d", &Escortships);
    ...
    while(j<=Escortships && !sankBattleship)
    {
        ...
        case 5:
            ...
            fprintf(escort,"                            Minimum Velocity: %d, Maximum Velocity - %d\n", Vmin, Vmax);
            fprintf(escort,"                            Minimum Angle: %d, Maximum Angle - %d\n", min_ang_e, max_ang_e);
            eship[j-1].impactPower = 0.04 * 100;
            printf("\n");
            break;
        default:
            printf("error");
...
// Check if the distance is within the maximum range of the escort ship's guns
            if (distance <= Rmax && distance >= Rmin)
            {
                ...
                while (battleshipHealth > 0)
                {
                    ...
                    battleshipHealth -= eship[j].impactPower;
                    ...
                    if (sankBattleship)
                    {
                        // mark the current escortship as destroyed
                        eship[count].isDestroyed = 1;
                        hitEscortships++;
                        printf("Escortship %d has been destroyed.\n", count + 1);
                        saveBattlefieldConditions(Escortships, x_battle, y_battle, canva_1, canva_2);
                        break;
                    }
                ...
                printf("Distance between the battleship and escortship: %f\n", distance);
                printf("Max Range of the escortship :%f\n\n", Rmax);
            }


            printf("Number of Escortships hit by the Battleship: %d\n", hitEscortships);

            int remainingEscortships;
            remainingEscortships = Escortships - hitEscortships ;
            printf ("remaining escortships %d \n",remainingEscortships);
