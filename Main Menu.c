...
float realtime(int Vmin ,float g);
...
int main()
{
    ...
    printf("5.Exit\n\n");
    printf("Select the needed option\n\n\n");
    int Mainmenu ;
    scanf("%d", &Mainmenu);
  int canva_1, canva_2,x_battle,y_battle,k;
...

{
    ...
    printf("The area of the canvas: %d\n", canva_size);
    ...
    float ang_B = 45.0; // Assuming a fixed angle
    float radian = ang_B * (PI / 180);
    ...
    float max_B_range = ((Vb*Vb) * sin(2 * radian)) /g;
    printf("Maximum Range of battleship shell: %f\n\n", max_B_range);
    fclose(Battleship);
...
// Check if the distance is within the range of the escort ship's guns
if (distance <= Rmax) {
 ...
 if (sankBattleship){
    printf("Battleship B was sunk by Escortship %d.\n", j);
 }

printf("STARTING SIMULATION 2\n");
    ...
    j=1;
     sankBattleship =0; //Flag to indicate if the battleship is sunk
...
//calling the array
    ...
    for (int count = 0; count <Escortships; count++) {
     //Calculate the distance between escortship and battleship
    if (eship[count].isDestroyed == 1)continue;
...
case 2:
                    printf("THE SIMULATION IS IN CASE 1\n");
        break;


case 3:
                     printf("Instructions\n");
                     printf("You have to enter non zero amount of escortships to continue the program \n");
                     printf("The canvas size should large enough to simulate the battle further \n");
                     printf("K points are randomly generated\n");
       break;
case 4:
    printf("SIMULATION STATS\n To get the accurate results , first go for battle simulation\n\n");
    printf("\n\nSimulating at point %d:(%d , %d)\n", k, x_battle, y_battle);
break;
